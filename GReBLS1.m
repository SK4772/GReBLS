function [] = GReBLS1(name,selectNum,N1,N2,Ng)
%对局部进行调参
%   此处显示详细说明
...
Ng=randi([20,40],1,1);
N2=randi([1500,3500],1,1);% number of enhancement nodes
s = .8;
%%两个参数的变化范围
...
save ( ['result\' char(name) '\train' num2str(select_num) '\Result_' char(name) '_' num2str(accuracy) '_' num2str(N1) '_' num2str(N2) '_' num2str(Ng) '_',num2str(select_num) '.mat'], 'accuracy', 'lambda', 'gamma', 'rate_acc','N1','N2','Ng');
clc
clear
close all
end
